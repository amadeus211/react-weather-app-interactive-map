...
export default function Weather({ setGeolocationLoaded }) {
  ...
  const fetchCity = async (lat, lon) => {
    try {
      ...
      const response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${process.env.REACT_APP_KEY}`
      );
      if (!response.ok) {
  ...
  const fetchCoords = async (city) => {
    try {
      ...
      const response = await fetch(
        `https://api.openweathermap.org/geo/1.0/direct?q=${city}&appid=${process.env.REACT_APP_KEY}`
      );
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
